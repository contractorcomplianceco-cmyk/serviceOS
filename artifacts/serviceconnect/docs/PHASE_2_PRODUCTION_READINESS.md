...
| Billing / AR (invoices, partial/credit/refund) | REAL (partial) | `balance = amount − amountPaid` holds and is server-enforced (overpayments and over-refunds rejected with 400); payment→refund round-trip tested; no GL |
