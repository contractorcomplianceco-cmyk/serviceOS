                ...
                return (
                  <div key={inv.id} className="p-4 hover:bg-white/[0.04] transition-colors" data-testid={`ar-${inv.id}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" className="flex-1 text-xs h-8 text-white blue-glow-soft" style={{background:'var(--sc-btn)',border:'1px solid var(--sc-btn-highlight)'}} onClick={() => { updateInvoice(inv.id, { status: "Paid", paidDate: new Date().toISOString() }); toast({ title: "Marked paid", description: `${inv.number} recorded as paid.` }); }} data-testid={`button-mark-paid-${inv.id}`}>
