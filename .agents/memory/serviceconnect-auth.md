...
## Seed
`artifacts/api-server/src/scripts/seed.ts` (`pnpm --filter @workspace/api-server run seed`). Tenant `org1`, 14 users u1–u14, `DEMO_PASSWORD="password123"`, idempotent (onConflictDoNothing). u1–u10 keep original ids/testids; u14 is a customer-portal user scoped to customerId=c1.
